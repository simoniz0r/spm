...
tarlistfunc () {
    if [ -z "$LISTPKG" ]; then
        ...
        dir -C -w 1 ~/.config/spm/tarinstalled | pr -tT --column=3 -w 125
        echo
        echo "$(cat ~/.config/spm/tar-pkgs.lst | wc -l) tar packages for install:"
        cat ~/.config/spm/tar-pkgs.lst | tr -d '"' | pr -tT --column=3 -w 125
    else
        if [ -f ~/.config/spm/tarinstalled/"$LISTPKG" ]; then
            echo "Current installed $LISTPKG information:"
            cat ~/.config/spm/tarinstalled/"$LISTPKG"
            echo "INSTALLED=\"YES\""
            echo "BIN_PATH=\"/usr/local/bin/$LISTPKG\""
        elif grep -q \"$LISTPKG\" ~/.config/spm/tar-pkgs.lst; then
...
tarlistinstalledfunc () {
    echo "$(dir -C -w 1 ~/.config/spm/tarinstalled | wc -l) tar packages installed:"
    dir -C -w 1 ~/.config/spm/tarinstalled | pr -tT --column=3 -w 125
    echo
    for tarpkg in $(dir -C -w 1 ~/.config/spm/tarinstalled); do
        echo "$tarpkg installed information:"
        cat ~/.config/spm/tarinstalled/"$tarpkg"
        echo "INSTALLED=\"YES\""
        echo
    done
}
...
tarsaveconffunc () {
    ...
    if [ "$TAR_DOWNLOAD_SOURCE" = "GITHUB" ]; then
        echo "TAR_GITHUB_COMMIT="\"$TAR_GITHUB_NEW_COMMIT\""" >> ~/.config/spm/"$SAVEDIR"
        echo "TAR_GITHUB_DOWNLOAD="\"$TAR_GITHUB_NEW_DOWNLOAD\""" >> ~/.config/spm/"$SAVEDIR"
        echo "TAR_GITHUB_VERSION="\"$TAR_GITHUB_NEW_VERSION\""" >> ~/.config/spm/"$SAVEDIR"
    fi
    echo "DESKTOP_FILE_PATH="\"$DESKTOP_FILE_PATH\""" >> ~/.config/spm/"$SAVEDIR"
    echo "ICON_FILE_PATH="\"$ICON_FILE_PATH\""" >> ~/.config/spm/"$SAVEDIR"
    echo "EXECUTABLE_FILE_PATH="\"$EXECUTABLE_FILE_PATH\""" >> ~/.config/spm/"$SAVEDIR"
    echo "CONFIG_PATH="\"$CONFIG_PATH\""" >> ~/.config/spm/"$SAVEDIR"
    echo "TAR_DESCRIPTION="\"$TAR_DESCRIPTION\""" >> ~/.config/spm/"$SAVEDIR"
    echo "DEPENDENCIES="\"$DEPENDENCIES\""" >> ~/.config/spm/"$SAVEDIR"
}

targithubinfofunc () {
    TAR_LATEST_RELEASE="$(wget --quiet "$TARURI" -O - | grep -i -m 1 '.*/download/*..*.tar*.' | cut -f2 -d'"' | sed 's:download:tag:g;' | cut -f-6 -d"/")"
    wget --quiet "https://www.github.com/$TAR_LATEST_RELEASE" -O ~/.config/spm/cache/"$TARPKG".latest
    TAR_GITHUB_NEW_DOWNLOAD="$(grep -i -m 1 '.*/download/*..*linux*..*64*..*tar*.' ~/.config/spm/cache/"$TARPKG".latest | cut -f2 -d'"')"
    TAR_GITHUB_NEW_COMMIT="$(grep '.*/commit//*.' ~/.config/spm/cache/"$TARPKG".latest | cut -f5 -d"/" | tr -d '>"')"
    TAR_GITHUB_NEW_VERSION="$(grep -i -m 1 '.*/download/*..*linux*..*64*..*tar*.' ~/.config/spm/cache/"$TARPKG".latest | cut -f2 -d'"' | cut -f6 -d"/")"
    TAR_DOWNLOAD_SOURCE="GITHUB"
    tarsaveconffunc "cache/$TARPKG.github"
    . ~/.config/spm/cache/"$TARPKG".github
    if [ -z "$TAR_GITHUB_NEW_DOWNLOAD" ]; then
        TAR_GITHUB_NEW_DOWNLOAD="$(grep -i -m 1 '.*/download/*..*linux*..*tar*.' ~/.config/spm/cache/"$TARPKG".latest | cut -f2 -d'"')"
        TAR_GITHUB_NEW_COMMIT="$(grep '.*/commit//*.' ~/.config/spm/cache/"$TARPKG".latest | cut -f5 -d"/" | tr -d '>"')"
        TAR_GITHUB_NEW_VERSION="$(grep -i -m 1 '.*/download/*..*linux*..*tar*.' ~/.config/spm/cache/"$TARPKG".latest | cut -f2 -d'"' | cut -f6 -d"/")"
        TAR_DOWNLOAD_SOURCE="GITHUB"
        tarsaveconffunc "cache/$TARPKG.github"
        . ~/.config/spm/cache/"$TARPKG".github
    fi
    if [ -z "$TAR_GITHUB_NEW_DOWNLOAD" ]; then
        TAR_GITHUB_NEW_DOWNLOAD="$(grep -i -m 1 '.*/download/*..*64*..*tar*.' ~/.config/spm/cache/"$TARPKG".latest | cut -f2 -d'"')"
        TAR_GITHUB_NEW_COMMIT="$(grep '.*/commit//*.' ~/.config/spm/cache/"$TARPKG".latest | cut -f5 -d"/" | tr -d '>"')"
        TAR_GITHUB_NEW_VERSION="$(grep -i -m 1 '.*/download/*..*64*..*tar*.' ~/.config/spm/cache/"$TARPKG".latest | cut -f2 -d'"' | cut -f6 -d"/")"
        TAR_DOWNLOAD_SOURCE="GITHUB"
        tarsaveconffunc "cache/$TARPKG.github"
        . ~/.config/spm/cache/"$TARPKG".github
    fi
    if [ -z "$TAR_GITHUB_NEW_DOWNLOAD" ]; then
        TAR_GITHUB_NEW_DOWNLOAD="$(grep -v '.*ia32*.'  ~/.config/spm/cache/"$TARPKG".latest | grep -i -m 1 '.*/download/*..*.tar*.' | cut -f2 -d'"')"
        TAR_GITHUB_NEW_COMMIT="$(grep -v '.*ia32*.'  ~/.config/spm/cache/"$TARPKG".latest | grep '.*/commit//*.' | cut -f5 -d"/" | tr -d '>"')"
        TAR_GITHUB_NEW_VERSION="$(grep -v '.*ia32*.'  ~/.config/spm/cache/"$TARPKG".latest | grep -i -m 1 '.*/download/*..*.tar*.' | cut -f2 -d'"' | cut -f6 -d"/")"
        TAR_DOWNLOAD_SOURCE="GITHUB"
        tarsaveconffunc "cache/$TARPKG.github"
        . ~/.config/spm/cache/"$TARPKG".github
    fi
...
tarappcheckfunc () { # check user input against list of known apps here
    if grep -q \"$1\" ~/.config/spm/tar-pkgs.lst; then
        KNOWN_TAR="TRUE"
        wget "https://raw.githubusercontent.com/simoniz0r/tar-pkg/master/apps/$1/$1.conf" -qO ~/.config/spm/cache/$1.conf
        . ~/.config/spm/cache/$1.conf
        TAR_DOWNLOAD_SOURCE="$DOWNLOAD_SOURCE"
        if [ "$TAR_DOWNLOAD_SOURCE" = "GITHUB" ]; then
            targithubinfofunc
        fi
    else
        if [ -f ~/.config/spm/tarinstalled/$1 ]; then
            . ~/.config/spm/tarinstalled/$1
        fi
        KNOWN_TAR="FALSE"
    fi
}
...
tarcustomdlfunc () {
    if [ -f ~/.config/spm/tarinstalled/$1 ]; then
        . ~/.config/spm/tarinstalled/$1
        cd ~/.config/spm/cache
        wget --quiet --read-timeout=30 --show-progress --trust-server-names "$TARURI" || { echo "wget $TARURI failed; exiting..."; rm -rf ~/.config/spm/cache/*; exit 1; }
        TARFILE="$(dir ~/.config/spm/cache/*.tar*)"
        TARFILE="${TARFILE##*/}"
    else
        ...
        case $TAR_DOWNLOAD_SOURCE in
            ...
            GITHUB)
                ...
                cd ~/.config/spm/cache
                wget --read-timeout=30 --quiet --show-progress "$TARURI" || { echo "wget $TARURI failed; exiting..."; rm -rf ~/.config/spm/cache/*; exit 1; }
                TARFILE="$(dir ~/.config/spm/cache/*.tar*)"
                TARFILE="${TARFILE##*/}"
                ;;
            DIRECT)
    ...
    if [[ "$KNOWN_TAR" != "FALSE" ]]; then
        case $TAR_DOWNLOAD_SOURCE in
            GITHUB)
                TARURI="https://github.com/$TAR_GITHUB_NEW_DOWNLOAD"
                cd ~/.config/spm/cache
                wget --quiet --read-timeout=30 --show-progress "$TARURI" || { echo "wget $TARURI failed; exiting..."; rm -rf ~/.config/spm/cache/*; exit 1; }
                ;;
            DIRECT)
                cd ~/.config/spm/cache
                wget --quiet --read-timeout=30 --show-progress --trust-server-names "$TARURI" || { echo "wget $TARURI failed; exiting..."; rm -rf ~/.config/spm/cache/*; exit 1; }
...
tarcheckfunc () {
    case $TARFILE in
        *tar.gz)
            tar -xvzf ~/.config/spm/cache/"$TARFILE" || { echo "tar $TARFILE failed; exiting..."; rm -rf ~/.config/spm/cache/*; exit 1; }
            ;;
        *tar.bz2|*tar.tbz|*tar.tb2|*tar)
            tar -xvf ~/.config/spm/cache/"$TARFILE" || { echo "tar $TARFILE failed; exiting..."; rm -rf ~/.config/spm/cache/*; exit 1; }
            ;;
        *)
...
tarupdateforcefunc () {
    if [ -f ~/.config/spm/tarinstalled/"$TARPKG" ]; then
        cat ~/.config/spm/tarinstalled/"$TARPKG"
    else
        echo "Package not found!"
        rm -rf ~/.config/spm/cache/*
    ...
    if [ -f ~/.config/spm/tarupgrades/$TARPKG ]; then
...
tarupgradecheckallfunc () {
    for package in $(dir -C -w 1 ~/.config/spm/tarinstalled); do
        TARPKG="$package"
...
tardesktopfilefunc () {
    case $1 in
        ...
        *)
            ...
            wget "https://raw.githubusercontent.com/simoniz0r/tar-pkg/master/apps/$TARPKG/$TARPKG.desktop" --show-progress -qO ~/.config/spm/cache/"$TARPKG".desktop  || { echo "wget $TARURI failed; exiting..."; rm -rf ~/.config/spm/cache/*; exit 1; }
            echo "Moving $TARPKG.desktop to $INSTDIR ..."
            sudo mv ~/.config/spm/cache/"$TARPKG".desktop "$INSTDIR"/"$TARPKG".desktop
            DESKTOP_FILE_PATH="$INSTDIR/$TARPKG.desktop"
            DESKTOP_FILE_NAME="$TARPKG.desktop"
            ;;
    esac
...
tarcustomappfunc () {
    ...
    EXTRACTED_DIR_NAME="$(ls -d ~/.config/spm/cache/*/)"
...
tarinstallstartfunc () {
    ...
    if [ "$KNOWN_TAR" = "FALSE" ];then
        ...
        cat ~/.config/spm/cache/$TARPKG.conf
        echo
...
tarupgradestartfunc () {
    ...
    case $UPGRADEANSWER in
        Y*|y*)
            ...
            tarcheckfunc
            tarupgradefunc
            rm ~/.config/spm/tarupgrades/"$TARPKG"
            ;;
        N*|n*)
...
tarremovefunc () {
    . ~/.config/spm/tarinstalled/"$REMPKG"
    ...
    if [ -f ~/.config/spm/tarupgrades/$REMPKG ]; then
        rm ~/.config/spm/tarupgrades/"$REMPKG"
    fi
    DESKTOP_FILE_NAME="$(basename "$DESKTOP_FILE_PATH")"
    ICON_FILE_NAME="$(basename "$ICON_FILE_PATH")"
    EXECUTABLE_FILE_NAME="$(basename "$EXECUTABLE_FILE_PATH")"
    ...
    rm ~/.config/spm/tarinstalled/"$REMPKG"
    echo "$REMPKG has been removed!"
}

tarremovepurgefunc () {
    . ~/.config/spm/tarinstalled/"$PURGEPKG"
    ...
    if [ -f ~/.config/spm/tarupgrades/$PURGEPKG ]; then
        rm ~/.config/spm/tarupgrades/"$PURGEPKG"
    fi
    DESKTOP_FILE_NAME="$(basename "$DESKTOP_FILE_PATH")"
    ICON_FILE_NAME="$(basename "$ICON_FILE_PATH")"
    EXECUTABLE_FILE_NAME="$(basename "$EXECUTABLE_FILE_PATH")"
    ...
    rm ~/.config/spm/tarinstalled/"$PURGEPKG"
